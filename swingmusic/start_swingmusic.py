...
from swingmusic.services.lyrics_backfill import backfill_library_async
...
from swingmusic.services.production_readiness import (
    ensure_lyrics_defaults,
    ensure_owner_and_projection,
)
from swingmusic.services.setup_state import resume_pending_index_if_needed
...
def start_swingmusic(host: str, port: int):
    ...
    def run_swingmusic():
        ...
        ensure_owner_and_projection()
        ensure_lyrics_defaults()
        backfill_library_async()
        resume_pending_index_if_needed()

        setproctitle.setproctitle(f"swingmusic {host}:{port}")
        start_cron_jobs()

        # Start unified queue worker.
        download_job_manager.start()
        playlist_tracking_service.start()
        print("Download services started")
